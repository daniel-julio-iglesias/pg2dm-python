...
class Classifier:
    ...
    def normalizeColumn2(self, columnNumber):
        """given a column number, normalize that column in self.data"""
        # first extract values to list
        col = [v[1][columnNumber] for v in self.data]
        # median = self.getMedian(col)
        ...
        maximum = max(col)
        dem = maximum - minimum
        print("Minimum: %f   dem = %f" % (minimum, dem))
        self.medianAndDeviation2.append((minimum, dem))
        for v in self.data:
            v[1][columnNumber] = (v[1][columnNumber] - minimum) / dem


    def normalizeVector2(self, v):
        """We have stored the median and asd for each column.
        We now use them to normalize vector v"""
        vector = list(v)
        for i in range(len(vector)):
            (minimum, dem) = self.medianAndDeviation2[i]
            # TODO
            vector[i] = (vector[i] - minimum) / dem
        return vector
    ...
    def classify2(self, itemVector):
        """Return class we think item Vector is in"""
        return(self.nearestNeighbor(self.normalizeVector2(itemVector))[1][0])
...
def test2(training_filename, test_filename):
    """Test the classifier on a test set of data"""
    classifier = Classifier(training_filename, 2)
    f = open(test_filename)
    lines = f.readlines()
    f.close()
    numCorrect = 0.0
    for line in lines:
        ...
        for i in range(len(classifier.format)):
              if classifier.format[i] == 'num':
                  ...
                  classInColumn = i
        theClass= classifier.classify2(vector)
        prefix = '-'
        if theClass == data[classInColumn]:
            # it is correct
            numCorrect += 1
...
if __name__ == '__main__':        
    ...
    # br = ('Basketball', [72, 162], ['Brittainey Raven'])
    # brNorm = c.normalizeVector(br[1])
    # brNorm2 = c.normalizeVector2(br[1])
    # print(brNorm)
    # print(brNorm2)

    # test2('..\\data\\ch4\\athletesTrainingSet.txt', '..\\data\\ch4\\athletesTestSet.txt')
    test2("..\\data\\ch4\\irisTrainingSet.data", "..\\data\\ch4\\irisTestSet.data")
    # test2("..\\data\\ch4\\mpgTrainingSet.txt", "..\\data\\ch4\\mpgTestSet.txt")
